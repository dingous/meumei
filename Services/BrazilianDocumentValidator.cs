...
public static class BrazilianDocumentValidator
{
    ...
    public static bool IsValidCnpj(string? value)
    {
        ...
        if (cnpj[..12].All(c => c == cnpj[0]))
            return false;

        var first = CalculateCnpjDigit(cnpj.AsSpan(0, 12), CnpjFirstWeights);
        var second = CalculateCnpjDigit(cnpj.AsSpan(0, 12), first, CnpjSecondWeights);
    ...
    public static bool IsValidCpf(string? value)
    {
        ...
        if (cpf.Length != 11 || !cpf.All(char.IsAsciiDigit))
            return false;
    ...
    private static int CalculateCnpjDigit(ReadOnlySpan<char> baseCnpj, IReadOnlyList<int> weights)
    {
        var sum = 0;
    ...
    private static int CalculateCnpjDigit(ReadOnlySpan<char> baseCnpj, int firstDigit, IReadOnlyList<int> weights)
    {
        var sum = 0;
        for (var i = 0; i < baseCnpj.Length; i++)
            sum += CnpjValue(baseCnpj[i]) * weights[i];
